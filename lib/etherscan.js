...
export function analyzeContract(contractSource, tokenInfo, holders, txCount, address) {
  const flags = []
  let riskScore = 0 // Start at 0, only add risk for bad signals
  ...
  if (hasSellRestriction) riskScore += 25
  ...
  // Bonus: verified + no red flags = lower score
  if (isVerified && !hasOwnerFunction && !hasSellRestriction && !(hasMint && !hasCap)) {
