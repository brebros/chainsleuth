...
// Public RPCs — fallback when Etherscan free API doesn't support proxy actions
const PUBLIC_RPCS = {
...
// Direct RPC call — fallback when Etherscan free API doesn't support chain
...
export function analyzeContract(contractSource, tokenInfo, holders, txCount, address) {
  ...
  // Proxy contracts use delegatecall by design — don't flag it
  const contractName = contractSource?.result?.[0]?.ContractName || ''
  const isLikelyProxy = contractName.toLowerCase().includes('proxy') ||
    sourceCode.includes('ERC1967') || sourceCode.includes('UUPSUpgradeable')
  const realSuspHits = isLikelyProxy ? suspHits.filter(p => p !== 'delegatecall') : suspHits
  if (realSuspHits.length > 0) {
    riskScore += 15
    flags.push({ name: 'Dangerous Code', status: 'danger',
      details: 'Found: ' + realSuspHits.join(', ') + ' — potential exploit vector',
  ...
  } else if (suspHits.length > 0 && isLikelyProxy) {
    flags.push({ name: 'Dangerous Code', status: 'safe',
      details: 'delegatecall found (standard proxy pattern — safe)',
      explanation: 'The contract uses delegatecall, which is standard for upgradeable proxy contracts (ERC-1967, UUPS). This is expected behavior, not a security risk.' })
  }
...
export async function getContractInfo(address, apiKey, chainId) {
  ...
  // Get tx count — fallback to RPC if Etherscan free API doesn't support chain
  try {
    ...
    if (txData?.result) {
      ...
      const rpcCount = await rpcCall(chainId, 'eth_getTransactionCount', [address, 'latest'])
      if (rpcCount) info.txCount = parseInt(rpcCount, 16)
    }
  } catch (e) {
    // Fallback: direct RPC call
    try {
  ...
  try {
    ...
    if (code?.result?.includes('363d3d373d3d3d363d73') || 
        code?.result?.includes('363d3d3d3d3d3d3d3d73') ||
        code?.result?.startsWith('0x363d3d373d3d3d363d73')) {
      info.isProxy = true
    }
  } catch (e) {}
