...
export function analyzeContract(contractSource, tokenInfo, holders, txCount, address) {
  ...
  const ponziPatterns = ['ponzi', 'pyramid', 'referral', 'staking requirement', '100%', 'hour']
  const ponziHits = ponziPatterns.filter(p => sourceCode.toLowerCase().includes(p))
  if (ponziHits.length >= 2 && !isVerified) {
    riskScore += 30
