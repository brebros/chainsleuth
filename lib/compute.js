...
class ZGCompute {
  ...
  initialize() {
    try {
      ...
      // OpenAI client appends /chat/completions to baseURL
      // 0G endpoint needs: .../proxy/v1/chat/completions
      const url = baseURL.endsWith('/v1') ? baseURL : baseURL + '/v1'

      console.log('0G Compute config:', { url, keyPrefix: apiKey.substring(0, 15) + '...', model: this.model })
  ...
  async analyzeContract(contractData) {
    ...
    try {
      ...
      console.error('0G Compute error details:', JSON.stringify({
        status: error.status,
        code: error.code,
        type: error.type,
